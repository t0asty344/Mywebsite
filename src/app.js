...
async function creatingtext() {
  ...
  if (isreplying) {
    const { error } = await supabase.from("testmessages").insert({
      message: inputs.value,
      user: localStorage.getItem("name"),
      hasreply: true,
      replytext: righclickedtext,
    });
    if (error) {
      console.log(error);
    }
...
async function realtimemessage() {
  ...
  data.forEach((da) => {
    createelements(da.message, da.user, da.hasreply, da.replytext, true);
  });
  if (error) {
    console.log(error);
  }
...
roomOne
  .on(
    "postgres_changes",
    { event: "INSERT", schema: "public", table: "testmessages" },
    (payload) => {
      realtimemessage();
    }
  )
...
function scroll() {
  ...
  textboxheight();
  textboxesarray.forEach((textboxes) => {
    heighttb += textboxes;
  });
...
function hide(id) {
  ...
  if (ids.classList.contains("hidden")) ids.classList.remove("hidden");
  else {
...
document.getElementById("username_display").innerHTML = escapeHTML(
  "username: " + usernamestor
);
document.getElementById("username_profil").innerHTML = escapeHTML(
  "username:<br>" + usernamestor
);

document.getElementById("scrolldown").addEventListener("click", scroll);
document
  .getElementById("btnusernamechange")
...
document
  .getElementById("activatechangebiodiv")
...
emoji.addEventListener("emoji-click", (event) => {
  inputs.value += event.detail.unicode;
});
